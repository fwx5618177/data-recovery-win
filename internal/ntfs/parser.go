...
const (
	...
	// Microsoft Basic Data 分区 GUID（混合字节序）
	// EBD0A0A2-B9E5-4433-87C0-68B6B72699C7
	// 注意: GUID 前三段是小端，后两段是大端
...
type DataRun struct {
	ClusterOffset int64 // 绝对簇号（已累加前序偏移）
...
func (s *Scanner) RecoverFile(
	...
	for _, run := range entry.DataRuns {
		// 稀疏运行（ClusterOffset == 0 表示稀疏/未分配）
		// 注意: 真正的第一个 run 的 ClusterOffset 可以是 0（磁盘开头），
		// 但这在实际 NTFS 中极罕见，通常簇 0 被引导扇区占用
...
func filetimeToTime(ft int64) *time.Time {
	...
	// 基准时间: 1601-01-01 00:00:00 UTC
	base := time.Date(1601, 1, 1, 0, 0, 0, 0, time.UTC)

	// 转换: 每个 FILETIME 单位 = 100 纳秒
	d := time.Duration(ft) * 100 // 100 纳秒 * ft = 纳秒数
	t := base.Add(d)
